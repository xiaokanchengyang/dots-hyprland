async function makeAmplitude() {
  ...
  var validatePropertyValue = function validatePropertyValue(key, value) {
    ...
    if (invalidValueTypes.indexOf(valueType) !== -1) {
      log.warn(
        'WARNING: Property key "' + key + '" with invalid value type ' + valueType + ', ignoring',
      );
      value = null;
      ...
      for (var i = 0; i < value.length; i++) {
        ...
        if (elemType === 'array') {
          log.warn(
            'WARNING: Cannot have ' + elemType + ' nested in an array property value, skipping',
          );
          continue;
  ...
  var options = function options(opts) {
    ...
    var domain = !utils.isEmptyString(opts.domain)
      ? opts.domain
      : '.' + topDomain(getLocation().href);
  ...
  function getAllItemFromChromeStorage() {
  ...
  function setItemToChromeStorage(key, value) {
    return new Promise((resolve) => {
  ...
  function removeItemToChromeStorage(key) {
    return new Promise((resolve) => {
  ...
  class ChromeStorageBuffer {
    constructor(chromeStorage = {}) {
      this.chromeStorageBuffer = JSON.parse(JSON.stringify(chromeStorage))
    }

    getItem(k) {
      return this.chromeStorageBuffer[k];
    }

    setItem(k, v) {
      setItemToChromeStorage(k, v);
      this.chromeStorageBuffer[k] = v;
    }

    removeItem(k) {
      removeItemToChromeStorage(k);
      delete this.chromeStorageBuffer[k];
    }
  }

  {
    // test that chrome storage is available and works
    var isChromeStorageAvailable = browser?.storage?.local && chrome.runtime.getManifest().manifest_version > 1;

    if (isChromeStorageAvailable) {
      localStorage = new ChromeStorageBuffer(await getAllItemFromChromeStorage());
    }

    // test that globalThis.localStorage is available and works
    ...
    if (!isChromeStorageAvailable && windowLocalStorageAvailable()) {
      localStorage = globalThis.localStorage;
    } else if (globalThis.globalStorage) {
      ...
      this.storage = {
        ...
        options: function options(opts) {
          ...
          this._options.domain =
            opts.domain ||
            this._options.domain ||
            (globalThis && globalThis.location && globalThis.location.hostname);
  ...
  Identify.prototype.add = function (property, value) {
    if (type(value) === 'number' || type(value) === 'string') {
      ...
      utils.log.error(
        'Unsupported type for value: ' + type(value) + ', expecting number or string',
      );
    }
  ...
  Identify.prototype._addOperation = function (operation, property, value) {
    ...
    if (this.userPropertiesOperations.hasOwnProperty(AMP_OP_CLEAR_ALL)) {
      utils.log.error(
        'This identify already contains a $clearAll operation, skipping operation ' + operation,
      );
      return;
  ...
  Request.prototype.send = function (callback) {
    ...
    if (isIE) {
      ...
      // XMLHttpRequest가 mv3에서는 동작하지 않아 fetch로 교체
      fetch(this.url, {
        method: 'POST',
  ...
  Revenue.prototype.setRevenueType = function setRevenueType(revenueType) {
    if (type(revenueType) !== 'string') {
      utils.log.error(
        'Unsupported type for revenueType: ' + type(revenueType) + ', expecting string',
      );
    } else {
  ...
  Revenue.prototype.setEventProperties = function setEventProperties(eventProperties) {
    if (type(eventProperties) !== 'object') {
      utils.log.error(
        'Unsupported type for eventProperties: ' + type(eventProperties) + ', expecting object',
      );
    } else {
  ...
  var uaParser = createCommonjsModule(function (module, exports) {
    ...
    (function (globalThis, undefined$1) {
      //////////////
      // Constants
      /////////////
      ...
      var UAParser = function (uastring, extensions) {
        ...
        var ua =
          uastring ||
          (globalThis && globalThis.navigator && globalThis.navigator.userAgent
      ...
      var $ = globalThis && (globalThis.jQuery || globalThis.Zepto);
      if ($ && !$.ua) {
    ...
    })(typeof globalThis === 'object' ? globalThis : commonjsGlobal);
  });
  ...
  var AmplitudeClient = function AmplitudeClient(instanceName) {
    this._instanceName = utils.isEmptyString(instanceName)
  ...
  AmplitudeClient.prototype.init = function init(apiKey, opt_userId, opt_config, opt_callback) {
    ...
    try {
      ...
      this._storageSuffix =
        '_' +
        apiKey +
        (this._instanceName === Constants.DEFAULT_INSTANCE ? '' : '_' + this._instanceName);
      ...
      this._apiPropertiesTrackingOptions =
        Object.keys(trackingOptions).length > 0
          ? {
      ...
      var initFromStorage = function initFromStorage(storedDeviceId) {
        _this.options.deviceId = _this._getInitialDeviceId(
          opt_config && opt_config.deviceId,
          storedDeviceId,
        );
        _this.options.userId =
        ...
        if (
          !_this._sessionId ||
          !_this._lastEventTime ||
          now - _this._lastEventTime > _this.options.sessionTimeout
        ) {
      ...
      if (AsyncStorage) {
        this._migrateUnsentEvents(function () {
          Promise.all([
            AsyncStorage.getItem(_this._storageSuffix),
            AsyncStorage.getItem(_this.options.unsentKey + _this._storageSuffix),
            AsyncStorage.getItem(_this.options.unsentIdentifyKey + _this._storageSuffix),
          ])
            .then(function (values) {
              ...
              if (_this.options.saveEvents) {
                _this._unsentEvents = _this
                  ._parseSavedUnsentEventsString(values[1])
                  .map(function (event) {
                    return {
                      event: event,
                    };
                ...
                _this._unsentIdentifys = _this
                  ._parseSavedUnsentEventsString(values[2])
                  .map(function (event) {
                    return {
                      event: event,
                    };
      ...
      .then(function (values) {
        if (_this2.options.saveEvents) {
          ...
          if (!!unsentEventsString) {
            itemsToSet.push(
              AsyncStorage.setItem(
                _this2.options.unsentKey + _this2._storageSuffix,
                JSON.stringify(unsentEventsString),
              ),
            );
          ...
          if (!!unsentIdentifyKey) {
            itemsToSet.push(
              AsyncStorage.setItem(
                _this2.options.unsentIdentifyKey + _this2._storageSuffix,
  ...
  AmplitudeClient.prototype._trackParamsAndReferrer = function _trackParamsAndReferrer() {
    ...
    if (this.options.logAttributionCapturedEvent) {
      var attributionProperties = _objectSpread(
        {},
        utmProperties,
        referrerProperties,
        gclidProperties,
  ...
  AmplitudeClient.prototype._apiKeySet = function _apiKeySet(methodName) {
    if (utils.isEmptyString(this.options.apiKey)) {
      utils.log.error(
        'Invalid apiKey. Please set a valid apiKey with init() before calling ' + methodName,
      );
      return false;
    }
  ...
  AmplitudeClient.prototype._getDeviceIdFromUrlParam = function _getDeviceIdFromUrlParam(
    urlParams,
  ) {
  ...
  AmplitudeClient.prototype.saveEvents = function saveEvents() {
    try {
      var serializedUnsentEvents = JSON.stringify(
        this._unsentEvents.map(function (_ref) {
    ...
    try {
      var serializedIdentifys = JSON.stringify(
        this._unsentIdentifys.map(function (unsentIdentify) {
  ...
  AmplitudeClient.prototype.setUserProperties = function setUserProperties(userProperties) {
    ...
    if (
      ...
      !utils.validateInput(userProperties, 'userProperties', 'object')
    ) {
      return;
  ...
  AmplitudeClient.prototype.identify = function (identify_obj, opt_callback) {
    ...
    if (identify_obj instanceof Identify) {
      ...
      utils.log.error(
        'Invalid identify input type. Expected Identify object but saw ' + type(identify_obj),
      );
  ...
  AmplitudeClient.prototype.groupIdentify = function (
    ...
    if (
      !utils.validateInput(group_type, 'group_type', 'string') ||
    ...
    if (identify_obj instanceof Identify) {
      ...
      if (Object.keys(identify_obj.userPropertiesOperations).length > 0) {
        return this._logEvent(
          ...
          null,
          null,
          _defineProperty({}, group_type, group_name),
          identify_obj.userPropertiesOperations,
          null,
          opt_callback,
        );
  ...
  AmplitudeClient.prototype._logEvent = function _logEvent(
    ...
    try {
      ...
      var event = {
        ...
        version_name: _shouldTrackField(this, 'version_name')
          ? this.options.versionName || versionName || null
          : null,
        platform: _shouldTrackField(this, 'platform') ? this.options.platform : null,
        os_name: _shouldTrackField(this, 'os_name') ? osName || null : null,
        os_version: _shouldTrackField(this, 'os_version') ? osVersion || null : null,
        device_model: _shouldTrackField(this, 'device_model') ? deviceModel || null : null,
        device_manufacturer: _shouldTrackField(this, 'device_manufacturer')
  ...
  AmplitudeClient.prototype.logEventWithTimestamp = function logEvent(
    ...
    if (this._shouldDeferCall()) {
      return this._q.push(
        ['logEventWithTimestamp'].concat(Array.prototype.slice.call(arguments, 0)),
      );
    }
  ...
  AmplitudeClient.prototype.logRevenueV2 = function logRevenueV2(revenue_obj) {
    ...
    if (revenue_obj instanceof Revenue) {
      ...
      utils.log.error(
        'Invalid revenue input type. Expected Revenue object but saw ' + type(revenue_obj),
      );
    }
  };

  {
    ...
    AmplitudeClient.prototype.logRevenue = function logRevenue(price, quantity, product) {
      ...
      if (
        ...
        !_isNumber(price) ||
        (quantity !== undefined && !_isNumber(quantity))
      ) {
  ...
  AmplitudeClient.prototype.removeEvents = function removeEvents(
    maxEventId,
    maxIdentifyId,
    status,
    response,
  ...
  AmplitudeClient.prototype.sendEvents = function sendEvents() {
    ...
    this._sending = true;
    var protocol = this.options.forceHttps
      ? 'https'
    ...
    var events = JSON.stringify(
      mergedEvents.eventsToSend.map(function (_ref2) {
  ...
  AmplitudeClient.prototype._mergeEventsAndIdentifys = function _mergeEventsAndIdentifys(
    numEvents,
  ) {
...

  {
     ...
     * @deprecated
     */
    AmplitudeClient.prototype.setGlobalUserProperties = function setGlobalUserProperties(
      userProperties,
    ) {
...

  {
    ...
    Amplitude.prototype.logEventWithGroups = function (
      eventType,
      eventProperties,
      groups,
      opt_callback,
    ) {
      return this.getInstance().logEventWithGroups(
        eventType,
        eventProperties,
        groups,
        opt_callback,
