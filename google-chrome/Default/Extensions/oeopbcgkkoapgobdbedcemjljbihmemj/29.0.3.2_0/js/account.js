...
function Account() {
   ...
   this.enablePushNotifications = async function(params = {}) {
       ...
       console.log("enablePushNotifications2: " + email);
        ...
        if (accountAddingMethod == "autoDetect" && !oauthOnlyActions) {
            ...
            if (accountAddingMethod == "autoDetect" && oauthOnlyActions) {
                try {
                    ...
                    if (error.message == JError.NO_TOKEN_FOR_EMAIL || error.code == ErrorCodes.BAD_REQUEST) {
   ...
   async function generateExecuteGmailAPIActionParams(params) {
	   ...
	   if (await account.getSetting("showSignature", "accountsShowSignature")) {
		   const sendAsData = await account.getSetting("sendAsData");
		   if (sendAsData) {
			   let signature;
			   const sendAs = sendAsData.sendAs.find(sendAs => sendAs.sendAsEmail == fromObj.email);
               if (sendAs) {
   ...
   this.fetchGmailSettings = async function(params = {}) {
        ...
        if (lastFetchedGmailSettings && lastFetchedGmailSettings.diffInDays() > -FETCH_INTERVAL_IN_DAYS) { // !params.refresh && 
            ...
            const data = await fetchText(that.getMailUrl());
   ...
   async function fetchLabelsFromHtmlSource() {
        ...
        try {
            ...
            const data = await fetchText(that.getMailUrl({urlParams:"ui=2&view=omni&rt=j&ik=" + idKey})); // "ui=2&view=up&rt=c&ik=" + idKey
            const labelStartStr = '[[[';
