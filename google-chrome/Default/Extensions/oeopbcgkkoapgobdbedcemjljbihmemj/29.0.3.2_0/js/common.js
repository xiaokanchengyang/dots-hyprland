...
function displayUncaughtError(error) {
    const errorStr = error.stack || error.message || error;
    ...
    if (!inLocalExtension && messageSendingError) {
        // do nothing in prod
        return;
    }
	...
	if (globalThis.polymerPromise2?.then) {
		polymerPromise2.then(() => {
			if (globalThis.showError) {
                ...
                if (error.cause == ErrorCause.DB_ERROR || indexedDBError || bucketError) {
                    showError(error, {
                        text: "See solution",
                        onClick: () => {
	...
	this.execute = async function(sendOAuthParams) {
        if (httpRequests.length) {
            ...
            for (const httpRequest of httpRequests) {
                ...
                addDataLine("");
                addDataLine(httpRequest.method + " " + httpRequest.path);
            }
            addDataLine("--" + boundary + "--");
...
function OAuthForDevices(defaultParams) {
	...
	this.openPermissionWindow = async function(params = {}) {
        const scopes = params.scopes || defaultParams.scopes;
        const stateParams = `${chrome.runtime.getURL(`oauth2callback.html?security_token=${await that.generateSecurityToken()}`)}&scopes=${scopes.join(" ")}`;

        var url = `${GOOGLE_AUTH_URL}?response_type=code&client_id=${GOOGLE_CLIENT_ID}&redirect_uri=${GOOGLE_REDIRECT_URI}&scope=${encodeURIComponent(scopes.join(" "))}&state=${encodeURIComponent(stateParams)}`;
	...
	this.getAccessToken = async function(params) {
        ...
        if (params.code) {
            ...
            tokenResponse.scopes = tokenResponse.scope.split(" ");
            ...
            tokenResponse.access_token = authResponse.token;
            tokenResponse.scopes = authResponse.grantedScopes;
        }
	...
	return { // public interface
		...
		save: async function(reason) {
            if (chrome.storage.sync) {
                ...
                syncOptions.exportIndexedDB({}, async function(exportIndexedDBResponse) {
		                ...
		                var handleResult = function(event) {  
		                	...
		                	if (cursor) {
		                		...
		                		if (cursor.key.startsWith("_") || (!params.exportAll && syncOptions.excludeList.includes(cursor.key))) {
                                    ...
                                    try {
                                        console.log("key: " + cursor.key + " length: ", cursor.value.value.length)
                                    } catch (error) {
    ...
    this.init = function () {
        ...
        if (that.loaded) {
            ...
            this.defaultsForOauth = {
                ...
                "poll": DetectClient.isFirefox() ? seconds(30) : "realtime"
            };
...
async function getInstanceToken() {
    return new Promise((resolve, reject) => {
        if (chrome.instanceID) {
            // seems this method can throw: Asynchronous operation is pending. ref: https://chromium.googlesource.com/chromium/src/+/34661418ac5f137f1dcb72194e76c117308b6c7f/chrome/browser/extensions/api/instance_id/instance_id_api.cc
