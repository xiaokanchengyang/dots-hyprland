    ...
    constructor() {
        ...
        this.getOTPCode = async function () {
            try {
                ...
                if (subject?.match(/\bcode|passcode|password\b/i) || subject?.match(/קוד/) || emailContent?.match(/\bsecurity code|one\-time passcode|confirmation code\b/i)) {
                    const regex = /\b([0-9]{6,8})\b/g;
                    ...
                    if (otpCodeMatch) {
                        ...
                        return await sendToOffscreenDoc("find-otp-code-in-html", {
                            ...
                            serializedRegex: serializedRegex
                        });
                    }
                }
            } catch (error) {
            ...
            if (that.account.getAccountAddingMethod() == "autoDetect") {
                ...
                if (contributors.length) {
                    ...
                    if (useMessages) {
                        ...
                        if (that.messages.first().from.email == lastContributor.email) {
