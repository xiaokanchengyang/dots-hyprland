	...
	return new Promise((resolve, reject) => {
        ...
        sendMessageToBG("mailAction", params, true).then(response => {
            ...
            if (error.cause?.spreauthUrl) {
                selectorAll("paper-toast").forEach(el => el.hide());
                //dismissToast(byId("undoToast"));
                //await niceAlert(error.message);
                await openGenericDialog({
                    okLabel: getMessage("signIn"),
...
async function letterAvatar(name, color) {
    ...
    //name = "oなßüab"
    if (!name || !/^[\p{L}\p{N}]/u.test(name.charAt(0))) {
        name = " ";
    }
...
function getAccountAvatar(account) {
	...
	Array.from(selectorAll(".accountAvatar")).some($accountAvatar => {
		if ($accountAvatar._account.email == account.email) {
...
function getAccountNodeByEmail(email) {
    ...
    for (let i = 0; i < $accountNodes.length; i++) {
        ...
        if ($accountNode._account.email === email) {
            return $accountNode;
        }
    }
    return null;
...
function initOpenEmailEventListeners() {
	...
	onClick(".listenToEmail, #menu-listen-to-email", async function() {
		const mail = getOpenEmail();
		const $mail = getMailNode(mail);
	...
	return new Promise(function(resolve, reject) {
		...
		mail.getThread({forceDisplayImages:mail.forceDisplayImages}).then(async response => {
			...
			if (mail.messages.last()) {
                    ...
                    chrome.permissions.contains({ origins: [Origins.EMAIL_ATTACHMENTS] }).then(hasPermission => {
                        if (hasPermission) {
                            ...
                            if (DetectClient.isFirefox()) {
                                ...
                                showToast({toastId: "show-calendar-details", duration: 5, text: getMessage("showCalendarDetails"), actionParams:{
                                    onClick: function() {
                                        chrome.permissions.request({ origins: [Origins.EMAIL_ATTACHMENTS] }).then(granted => {
                                            dismissToast(byId("show-calendar-details"));
                                            if (granted) {
                                                byId("openEmailProgress").classList.add("visible");
                ...
                onClickReplace($replyArea, async function() {
                    console.log("replyTextarea clicked");
					if (!$replyArea.classList.contains("clicked")) {

                        if (accountAddingMethod == "autoDetect") {
                            const tokenResponse = await oAuthForEmails.findTokenResponse({ userEmail: mail.account.getEmail() });
                            if (!tokenResponse) {
            ...
            sleep(150).then(() => {
                requestIdleCallback(async () => {
                    ...
                    if (otpCode) {
                        ...
                        if (!Array.from(selectorAll("paper-toast")).some(el => isVisible(el))) {
                            showMessage(otpCode, {
...
if (chrome.runtime.onMessage) {
	chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
		if (message.command == "profileLoaded") {
            ...
            sendResponse();
        } else if (message.command == "refreshPopup" || message.command == "refreshSidePanel") {
            location.reload();
            sendResponse();
        ...
        /*
            ...
            showMessage(getMessage("newEmail"), {
...
function openOtherMail($mail, direction) {
	...
	if (nextMail?.account && mailAccount && mailAccount.email == nextMail.account.email) {
    ...
    showMessage("", {
        ...
        onClick: async () => {
            ...
            for (const email in accountsCheckingSpam) {
                const account = accounts.find(account => account.getEmail() == email);
...
function setAccountHeaderColor($account, color) {
    let accountHeaderStyle = $account.querySelector(".accountHeader").getAttribute("style");
    if (accountHeaderStyle) {
        accountHeaderStyle += `; `;
    } else {
        accountHeaderStyle = "";
    }
    accountHeaderStyle += `background-color: ${color}`;
    if (color != DEFAULT_SETTINGS.accountColor) {
        accountHeaderStyle += " !important";
    }
    $account.querySelector(".accountHeader").setAttribute("style", accountHeaderStyle);
}
		...
		if (accountIndex != 0 && accountIndex == (accounts.length-1) && !account.hasBeenIdentified()) {
			...
			$account.querySelector(".accountOptionsMenuButton").addEventListener("mouseup", async function() { // MUST use .one because mousedown will also be called when menu items *inside the dropdown all are also clicked
                ...
                if (accountsCheckingSpam && accountsCheckingSpam[account.getEmail()]) {
                    $account.querySelector(".checkSpam").classList.add("done");
                    $account.querySelector(".checkSpam iron-icon").setAttribute("icon", "check");
                }
...
function showSkinsDialog() {
	...
	Controller.getSkins().then(async skins => {
		...
		skins.forEach(skin => {
			...
			const addButton = document.createElement("paper-checkbox");
        ...
        onClickReplace($dialog.querySelector(".customSkin"), async function() {
            ...
            function canLoadBackgroundImage() {
                const imageUrl = $dialog.querySelector("#customBackgroundImageUrl").value;
                if (!imageUrl) {
                    return true;
                }
                return new Promise((resolve, reject) => {
            ...
            onClickReplace($dialog.querySelector(".updateSkin"), async function() {
                if (await canLoadBackgroundImage()) {
                    ...
                    addSkin({
                        id:"customSkin",
                        css: $dialog.querySelector("textarea").value,
                        image: $dialog.querySelector("#customBackgroundImageUrl").value
                    });
                    if (!await storage.get("donationClicked")) {
            ...
            onClickReplace($dialog.querySelector(".okDialog"), async function() {
                if (await canLoadBackgroundImage()) {
        ...
        onClickReplace($dialog.querySelector(".okDialog"), async function() {
            if (byId("previewSkin")) {
                ...
                if (attemptedToAddSkin) {
                    ...
                    const $icon = document.createElement("paper-checkbox");
                    $icon.setAttribute("disabled", "");
                    $icon.setAttribute("style", "margin-left:8px;vertical-align: middle");
                    
                    content.append("Use the checkbox", $icon, "to add skins!");
...
async function maybeRenderAccounts() {
    ...
    console.log("accountsMailCount", accountsMailCount);
    //previousMailIds = previousMailIds.filter(mailId => !hiddenMails.includes(mailId));
    await initAllAccounts(); // add with side panel feature; hope this doesn't cause sync issues
    const currentMailIds = getMailIds();
    console.log("accountsMailCountafter", accountsMailCount);

    console.log("prev", previousMailIds);
    console.log("newe", currentMailIds);
...
async function init() {
    ...
    if (skinsSettings) {
		docReady().then(async () => {
			skinsSettings.forEach(skin => {
                ...
                if (skin.id == SkinIds.HEADER_SMALL) {
                    polymerPromise.then(() => {
    ...
    if ($shareButton) {
        ...
        $shareButton.addEventListener("click", async function() {
            ...
            shareMenu.style.top = `${shareButtonRect.top}px`;
            shareMenu.style.left = `${shareButtonRect.left-10}px`;

            show("#share-menu");
            
            onClickReplace("#share-menu paper-item", event => {
